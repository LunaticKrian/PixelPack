...
| 文档 | 内容 |
|------|------|
| [docs/deploy.md](docs/deploy.md) | 完整部署流程：前端构建 → 后端部署 → 网关更新，含验证、日常更新、备份、故障速查 |
| [docs/technology/260719-nginx部署架构.md](docs/technology/260719-nginx部署架构.md) | 网关托管模式架构（统一入口、共享网络、通配证书、多项目接入） |
| [docs/technology/260719-新服务上线与网关扩展.md](docs/technology/260719-新服务上线与网关扩展.md) | 新项目接入 SOP：后端容器接入 `airise-web`、由 `_template.conf` 生成站点配置、扩展网关 |
...
| [nginx/README.md](nginx/README.md) | 网关镜像与容器（`airise-gateway`）说明 |
